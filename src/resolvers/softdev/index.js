...
module.exports = {
    createTodoList: async (req, res) => {
        ...
        const todolist = await knex('softdev').insert({
            id,
            title,
            description
        })
    ...
    getsTodoList: async (req, res) => {
        const todolist = await knex('softdev')
    ...
    getsTodoListByUser: async (req, res) => {
        ...
        const todolist = await knex('softdev').where('user_id', user_id)
        if (todolist.length == 0) return res.status(404).json({message: 'Todo List Kosong'})
        return res.status(200).json({data: todolist})
    },
    getTodoList: async (req, res) => {
        const { id } = req.params
        const todolist = await knex('softdev').where('id', id).first()
    ...
    updateTodoList: async (req, res) => {
        ...
        const result = validationResult(req)
        if (!result.isEmpty()) return res.status(400).json({errors: result.array()})
        const todolist = await knex('softdev').where('id', id).update({
            title,
            description
        })
    ...
    deleteTodoList: async (req, res) => {
        const { id } = req.params
        const todolist = await knex('softdev').where('id', id).del()
